...
/**
 * The bean class as return for remote API of GET /client/propbase/{projectName}.
 * @author FreeGen
 */
type ClientPropbaseResult = {
  ...
  /** そのDBFluteクライアントが利用するDBFluteのバージョン e.g. "1 (Required) */
  dbfluteVersion: string

  /** SchemaHTMLが自動生成されているかどうか？ e.g. false (Required) */
  hasSchemaHtml: boolean

  /** HistoryHTMLが自動生成されているかどうか？ e.g. false (Required) */
  hasHistoryHtml: boolean

  /** SchemaSyncCheckの結果HTMLが存在しているかどうか？ e.g. false (Required) */
  hasSyncCheckResultHtml: boolean

  /** AlterCheckの結果HTMLが存在しているかどうか？ e.g. false (Required) */
  hasAlterCheckResultHtml: boolean

  /** SchemaPolicyの違反がある状態かどうか？ (古いバージョンの場合はnull) e.g. false (NullAllowed) */
